function ctInitViewListing() {
			...
			.click(
					function() {
						...
						var name = $('.ctName').val(), email =
								...
								param = {};
						if (bmpUtil
								.isTextFieldEmpty('.ctIsChkEmptyEnquiryVal'))
						{
							param = {
							...
							ctDAO.addEnquiry(param, function(data){
								if(data && bmpUtil.RESPONSE_STATUS == data.responseStatus)
									{
										$('.ctEnquiryModalClose').trigger();
									}
								else
									{
	...
	$('.ctViewLisitingLoadMore')
			.click(
					function() {
						var currentPage =
								parseInt($('#ctViewListingNumEntries').val()), numEntries =
								parseInt($('#ctCommentsNumEntries').val()), params =
								{};
