...
function ctInsertTripData() {
			...
			parseInt(noDaysTxt) >= MIN_TRIP_DAYS)
	{
				...
				&& ctChkFileUploaded()) 
		{
			var ctRouteFormLen = $('.ctRouteFormDiv').length, 
					ctDurationLen = $('.ctItenaryParentDiv').length, 
					convertDateENG = bmpUtil.convertDateENG($("#fromDatePicker").val()), 
					location = $("#ctSelectActivityType").select2('data').text, 
					tripTitle = $('.ctVendorActivitySelectOPtion option:selected').text()
								+ ", "
								+ location
								+ " - "
			...
			$('.ctVendorListingForm').append(
					'<input type="hidden" class="ctTipTitle" value="' + tripTitle
							+ '" name="triptitle">');
			$('.ctPhotogalleryFileName').each(function() {
	...
	{
		sweetAlert("Oops...", "Please enter your days must be greater than zero!", "error");
		isNotEmpty = false;
	}
	return isNotEmpty;
}
